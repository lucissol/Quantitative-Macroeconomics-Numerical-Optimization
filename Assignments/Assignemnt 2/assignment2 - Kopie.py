...
policy_egm2_large = model2.solve_EGM(kgrid_large)
...
k_sim1, c_sim1, y_sim1, path_sim1 = model1.simulate_economy(kgrid, T, burnin, plot=True, initial_state_idx=4)
...
# distribution of capital stock is heavily skewed, since agent(s) use good time sto accumulate extreme asset holdings
#%% g)
...
# check that both time series were generated given the same shocks
print(f"Dynamic errors computed on the same shock path: {np.array_equal(real_econ[3], imp_econ[2])}")
